...
class GetSidebarMenu implements MenuInterface{
    ...
    /*
        * By ZKOF
    */
    private function getMenuFromDBbYzkof($menuId, array $menuNames){
        $this->menu = Menus::join('menu_role', 'menus.id', '=', 'menu_role.menus_id')
            ->select('menus.*',)
            ->where('menus.menu_id', '=', 3)
            ->whereIn('menu_role.role_name', $menuNames)
            ->groupBy('menus.href', 'menus.id')
    ...
    public function get($role, $menuId=3){
        $this->getMenuFromDBbYzkof($menuId, $role);
